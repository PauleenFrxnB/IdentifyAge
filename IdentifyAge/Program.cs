...
namespace IdentifyAge
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            IdentifyGeneration(age, ref generation);
            IdentifyStage(age, ref stage);

            Console.WriteLine($"{age} years old, {generation}, {stage}");
        }
        static void IdentifyGeneration(int age, ref string generation)
        {
            if (age >= 79 && age <= 96)
            {
        ...
        static void IdentifyStage(int age, ref string stage)
        {
            if (age >= 0 && age <= 3)
            {
                stage = "Infant";
